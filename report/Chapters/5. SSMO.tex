...
Let us now discuss the way an SSMO is actually implemented. The calculation of $z$ is straightforward, Equation \ref{eqn:ssmo-z} is used. the state estimates $\hat{x}^{\mcJ/\mcP}_{j/p}$ are calculated with the transformation matrices as follows
