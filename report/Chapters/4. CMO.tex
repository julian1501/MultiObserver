...
\begin{figure}[h]
    ...
    % System Block
    \node (system) [block, text width=4cm] {System \\ $\dot{x} = Ax + Bu + E\phi(y)$ \\ $y_i = C_ix + Du + \tau_i$\\$i=1,2,\dots,N_O$};
    
    % Outputs Block
    \node (outputs) [block, right=of system, text width=4cm] {Outputs \\ $y_1, y_2, \dots, y_{N_O}$};
    
    % Observers Section
    \node (Jobservers) [block, below=of outputs, xshift=4cm, text width=6cm] {$J$-Observers \\ $\dot{\hat{x}}_j^{\mcJ} = (A + L_j^{\mcJ}C_j^{\mcJ})\hat{x}^{\mcJ}_j - L_j^JC_j^{\mcJ}x + Bu + E\phi(y) - L_j^{\mcJ}(v_j^{\mcJ} + \tau_j^{\mcJ})$ \\ $j=1,2,\dots,N_J$};
    \node (Pobservers) [block, below=of outputs, xshift=-4cm, text width=6cm] {$P$-Observers \\ $\dot{\hat{x}}_p^{\mcP} = (A + L_p^{\mcP}C_p^{\mcP})\hat{x}_p^{\mcP} - L_p^{\mcP}C_p^{\mcP}x + Bu + E\phi(y) - L_p^{\mcP}(v_p^{\mcP} + \tau_p^{\mcP}).$ \\ $p=1,2,\dots,N_P$};
    
    % Comparison Block
    \node (comparison) [block, below=of Jobservers, xshift=-4cm, text width=4cm] {Comparison \\ $\pi_j = \max_{p \subset j} |\hat{x}_j - \hat{x}_p|$ \\ $\sigma = \arg \min \pi_{\mathcal{J}}$};
