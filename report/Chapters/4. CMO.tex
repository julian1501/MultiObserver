...
\begin{equation}\label{eqn:MO-size-requirement}
    2N_M<N_O,
\end{equation}
in other words: more than half the sensors need to remain attack free at all times. Consider system \eqref{eqn:standard-system} with $N_O$ outputs and no nonlinearity (i.e. $\phi(x)=0 \ \forall \ x \in \mathbb{R}^{n_x}$)
\begin{equation*}
    \begin{split}
    \dot{x} &= Ax + Bu \\
